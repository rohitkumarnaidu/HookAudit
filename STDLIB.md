...
| 7 | `mocha` / `jest` / `ava` | `node:test` + `node:assert/strict` | Node's built-in test runner (stable since v20) gave us subtests, TAP output, and `--test` file targeting with zero setup. All 9 of our tests run on it. |
