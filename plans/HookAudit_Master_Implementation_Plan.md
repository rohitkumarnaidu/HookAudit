...
**Build:** `bin/hookaudit.js` 1271 lines (now 60787 B after // noise filter), `node v24.19.0`, `npm ls --all → (empty)`, SHA256 `0AD6C16F31887E7DA17BCBE41CAFC9DDC33FC3D125260E7546E696FBAD73B744` `bin/hookaudit.js:15-18` (`node:fs`, `node:path`, `node:crypto`, `node:util` only)
...
|---|---|---|---|---|
| §5.0.1 | File-size guard `MAX_FILE_SIZE=1MiB → FILE_TOO_LARGE` | **COMPLETE** | `bin/hookaudit.js:24` `MAX_FILE_SIZE`, `bin/hookaudit.js:264,734,843` `lstat.size > MAX`, `readTextSafeWithGuards` | `large file: >1MiB is skipped` ✔ |
| §5.0.2 | Binary detection `BINARY_SKIPPED` | **COMPLETE** | `bin/hookaudit.js:26` `BINARY_CHECK_BYTES`, `bin/hookaudit.js:161` `isBinaryContent`, `bin/hookaudit.js:270` `BINARY_SKIPPED` | `binary file is skipped` ✔ |
| §5.0.3 | Symlink policy `lstat + SYMLINK_SKIPPED` | **COMPLETE** | `bin/hookaudit.js:218` `listFilesRecursive lstat`, `bin/hookaudit.js:495` `resolveSurfaceFiles lstat`, `bin/hookaudit.js:727` resolver `lstat+isSymbolicLink` | `symlink outside root is skipped` ✔ |
| §5.0.4 | Boundary helper `resolveInsideRepository` | **COMPLETE** | `bin/hookaudit.js:174` central helper (`path.resolve` + `path.relative` + `isAbsolute` + UNC + `C:\` drive, case-insensitive `startsWith`), single source of truth | `boundary traversal: ../ and absolute outside are flagged BOUNDARY_VIOLATION` ✔ |
| §5.0.5 | Cycle/depth primitives `visited + MAX_GRAPH_DEPTH=32` | **COMPLETE** | `bin/hookaudit.js:25` `MAX_GRAPH_DEPTH=32`, `bin/hookaudit.js:647` `visited:Set` + `visitedFiles:Set`, queue BFS `bin/hookaudit.js:769` | `cycle: A→B→C→A is detected with CYCLE_DETECTED` ✔ + depth guard `DEPTH_LIMIT_REACHED` `bin/hookaudit.js:779` |
| §6 | Evidence & Diagnostics Upgrade | **COMPLETE** | `bin/hookaudit.js:325` `createEvidence{path,field,detector,reason,excerpt}`, `bin/hookaudit.js:28` `DIAGNOSTIC_CODES` 13 codes, `bin/hookaudit.js:346` `computeConfidence` `HIGH/MED/LOW` | findings carry `field: hooks.SessionStart[0].hooks[0].command`, `evidence[]`, `diagnostics[]` sorted |
| §7 | Adapter Normalization (11 surfaces, `canHandle→parse→normalize`) | **COMPLETE** | `bin/hookaudit.js:62` `SURFACES[11]`, `bin/hookaudit.js:352,375,388` `extractClaudeHookCommands` + `extractVscodeTaskCommands` + `extractPackageJsonScripts`, Cursor distinguishes `instruction vs hook` (only documented hooks become surfaces), adapters never own risk | `clean/malicious` fixtures still pass 9 original tests |
| §8 | Reference Resolver CORE MVP `config→script→script` | **COMPLETE** | `bin/hookaudit.js:174` boundary, `bin/hookaudit.js:642` `resolveExecutionGraph` BFS queue, `bin/hookaudit.js:617` `extractScriptReferences` (`require/import/node/python/bash/source` + `./ ../ .claude/.vscode/scripts/`), baseDir-aware `bin/hookaudit.js:789` (`path.dirname(cur.abs)`), handles `process.env.X+"/setup.sh"` → `DYNAMIC_EXECUTION` | `multi-hop: config → script A → script B → network` ✔, `dynamic reference is flagged DYNAMIC_EXECUTION with LOW confidence` ✔ |
| §9 | Execution Graph nodes/edges | **COMPLETE** | `bin/hookaudit.js:642` nodes `REPOSITORY/CONFIG/TRIGGER/COMMAND/SCRIPT/FILE/CAPABILITY` (`repo 652`), `bin/hookaudit.js:925` edges `CONTAINS/TRIGGERS/EXECUTES/REFERENCES/CONNECTS_TO`, evidence per edge, deterministic sort `bin/hookaudit.js:938`, cross-tool `.claude→.vscode` statically supported | `graph.nodes 7 clean / 21 malicious`, `graph.edges`, `paths` verified `clean:2` `malicious:3` |
| §10 | Capability Engine P0/P1/P2 | **COMPLETE** | `bin/hookaudit.js:44` `CAPABILITY` 11-enum, `bin/hookaudit.js:82` `RULES[]` mapped `network-fetch→NETWORK_ACCESS`, `runtime-bootstrap→RUNTIME_BOOTSTRAP+REMOTE_DOWNLOAD`, `obfuscation→OBFUSCATION+DYNAMIC_EXECUTION`, `process-exec→PROCESS_EXECUTION`, `cross-reference→CROSS_TOOL_LINK`, plus `REMOTE_DOWNLOAD` vs `NETWORK_ACCESS` distinction | `malicious` shows `CROSS_TOOL_LINK, PROCESS_EXECUTION, NETWORK_ACCESS, RUNTIME_BOOTSTRAP, REMOTE_DOWNLOAD` |
| §11 | Unified Path Risk + Confidence | **COMPLETE** | `bin/hookaudit.js:466` `computePathRisk` deterministic table (`automatic+network+process→HIGH`, `automatic+remote-download+process+obfuscation→CRITICAL`), `bin/hookaudit.js:346` `computeConfidence`, `risk` ≠ malware (`HIGH-RISK EXECUTION PATH`) | `clean PostToolUse→MEDIUM`, `malicious SessionStart→HIGH`, `folderOpen→CRITICAL`, confidence `HIGH` literal vs `MEDIUM` resolved vs `LOW` dynamic |
| §12 | JSON Contract v1 | **COMPLETE** | `bin/hookaudit.js:1133` `printJson` envelope `{version:1, repository:{path,absolute}, summary:{executionSurfaces,withFindings,critical,warn,paths,highRiskPaths,diagnostics,decision:PASS|REVIEW|BLOCK}, surfaces[], paths[], graph:{nodes,edges}, capabilities[], diagnostics[], diff}` + `results/diff` backward-compat shim | `clean summary decision REVIEW`, `malicious decision BLOCK`, `version:1` verified |
| §13 | Human Report (path-first) | **COMPLETE** | `bin/hookaudit.js:1052` `printHuman` prioritizes `High-risk execution paths:` then findings then `Diagnostics` then `Drift`, shows `WHAT/WHEN/WHERE/PATH/CAPABILITY/WHY`, coverage note `Unsupported execution surfaces were not analyzed.` `bin/hookaudit.js:1100+`, no-finding wording `No high-risk execution paths detected in supported/analyzed surfaces.` | `human report priority: high-risk paths shown first` ✔ |
| §14 | Baseline/Diff | **COMPLETE** | `bin/hookaudit.js:958` `writeBaseline` `schemaVersion:2` `{files,surfaces,capabilitySummary,graphSummary}`, `bin/hookaudit.js:958` `readBaseline` legacy compat, `bin/hookaudit.js:984` `diffAgainstBaseline` `NEW/CHANGED/REMOVED` + semantic `NEW_TRIGGER/REMOVED_TRIGGER/NEW_COMMAND/NEW_CAPABILITY/REMOVED_SURFACE` via `surfaces` compare | `baseline then diff … no drift` ✔, `CHANGED` ✔, `capability diff: new NETWORK_ACCESS … NEW_CAPABILITY` ✔ |
...
| **Gate 5 Hackathon** | zero deps, one-command run, deps-proof, README/STDLIB/RULES/SECURITY/LIMITATIONS, public repo, license, tests, demo | **PASS** | `dependencies:{}` `npm ls --all → (empty)`, `node bin/hookaudit.js --help` one-command (+ `make` on Unix, `npm test` cross-platform), `deps-proof.txt` regenerated `0AD6C16F`, `README/STDLIB/RULES/SECURITY/LIMITATIONS` present, `LICENSE MIT`, `22/22 tests`, demo `demo/sample-repository` 3 surfaces 4 paths BLOCK |

**Zero-Dependency Proof:** `package.json:15-16` `dependencies:{}, devDependencies:{}`, `bin/hookaudit.js:15-18` 4 `node:` requires only, `npm ls --all` `(empty)`, `Select-String` shows no `child_process/vm/fetch/https/http/net/dns` runtime imports (only detector strings), `deps-proof.txt:SHA256 0AD6C16F…` `POSIX YES Deterministic YES`
