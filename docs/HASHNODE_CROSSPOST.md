---
title: "HookAudit: Building a Supply-Chain Security Scanner Without a Supply Chain"
subtitle: "What happens when you force a security tool to inspect untrusted code using only standard-library primitives? An engineering postmortem on systems complexity and zero dependencies."
slug: hookaudit-zero-dependency-security-scanner
tags: security, nodejs, javascript, opensource
canonical: "https://dev.to/bappadala_rohithkumarna/hookaudit-building-a-supply-chain-security-scanner-without-a-supply-chain-1aec"
enableToc: true
---
...
<img src="docs/images/hookaudit_cli_high_risk_scan.png" alt="HookAudit CLI High-Risk Scan" width="100%" />  
...
- The entire scanner runtime lives in a single file: `bin/hookaudit.js` (2,357 lines, SHA-256: `A3C45D82D526E1EE8B996853B58E355AAF2396EEDED227E7372C9E60E522829B`).
...
<img src="docs/images/hookaudit_zero_dep_proof.png" alt="Zero-Dependency and Native Test Runner Verification" width="100%" />  
...
<img src="docs/images/hookaudit_browser_topology_graph.png" alt="HookAudit Browser Topology Graph Visualization" width="100%" />  
...
4. **`node:crypto.randomUUID` & `createHash`**: Native RFC 4122 v4 UUID generation and SHA-256 stream hashing backed by OpenSSL.
...
3. **Packfile Delta Decompression**: `node:zlib` inflates loose objects, but Git packfile delta reconstruction would require thousands of lines of binary arithmetic. We supported loose objects and packed refs, flagging packfile-only histories as `UNSUPPORTED_FORMAT`.
...
- **The Never-Execute Guarantee**: HookAudit reads files strictly as inert UTF-8 text via `fs.readFileSync()`. It never invokes `eval()`, `new Function()`, `child_process.exec()`, or `vm.runInContext()` on target code. A dedicated regression test verifies this invariant.
...
<img src="docs/images/hookaudit_baseline_drift_diff.png" alt="HookAudit Baseline and Semantic Drift Diff" width="100%" />  
...
By eliminating external dependencies, HookAudit achieves self-contained auditability, immutable reproducibility, and safe evaluation of hostile repositories without running package-manager install hooks on the target code.
